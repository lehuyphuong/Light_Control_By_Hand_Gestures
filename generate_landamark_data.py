...
class HandDatasetWriter():
    def __init__(self, filepath) -> None:
        ...
        self.file_writer = csv.writer(self.csv_file, delimiter=',', quotechar='|',
...
def run(data_path, sign_img_path, split="val", resolution=(1280, 720)):
    ...
    while cam.isOpened():
        ...
        if (current_letter is None):
            ...
            if label == -65:
                status_text = f"Recoding unknown, press spacebar again to stop"
                label = -1

            else:
                status_text = f"Recoding {LABEL_TAG[label]}, press {current_letter} again to stop"
        ...
        if (cannot_switch_char):
            cv2.putText(annotated_image, f"please press {current_letter} again to unbind", (
                9, 450), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
